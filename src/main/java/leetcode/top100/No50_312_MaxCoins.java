...
public class No50_312_MaxCoins {
     ...
     * 2）第二步，从区间长度为2开始，逐步求解到区间长度为新数组length，因为你要求解的是整个区间，左侧的区间长度最大是length -1。
     ...
     *
     * 认知突破：
     * 本质上动态规划就是和暴力递归一样遍历的所有的可能性，只不过它比暴力递归存储了计算过的值，避免了重复计算，且使用了递推的方式来计算。
     * 它的遍历所有可能性的行为藏在它的转移公式当中，一般会经过比较和判断，取出最大值和最小值。
     * 所以最常规的思路都是使用暴力递归的方式，通过倒推来找到转移公式，然后改成递推方式求解结果。这是最稳妥的方式。
     */
